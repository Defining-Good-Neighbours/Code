...
LenMax = Constant(Len0 + 2*GpD)     # Max length in 2 days

# Create mesh and define function space
nx = ny = 100
mesh = RectangleMesh(Point(a,-(LenMax+0.3)), Point(xr, 0), nx, ny) # x-axis = r, y-axis = z
...
indic1 = Expression('(near(x[0], a, tol) and x[1] <= 0 and x[1] >= -(Len0 + G*t) ) ? 1: 0', degree=1, a=a, tol=tol, Len0=Len0, G=G, t=0)
# indic1 = Expression('near(x[0], a, tol) ? 1: 0', degree=0, a=a, tol=tol)
indic2 = Expression('(near(x[0], a, tol) and x[1] <= -(Len0 + G*t-dLenY) and x[1] >= -(Len0 + G*t) ) ? 1: 0', degree=1, a=a, tol=tol, Len0=Len0, G=G, dLenY=dLenY, t=0)
