...
δlt = 0.2                       # excudation 2 cm behind the tip
...
xr = (xr-a)/ε_r;                                    # Translation in a (not used here)
...
#initialising Mesh
#domain = MeshFunction("size_t", mesh, 2)
...
ν_vec = Constant([ν_r,ν_z])
def vec1(X):
    return Dx * grad(X) - X * ν_vec
def vec2(Y):
    return Dy * grad(Y) - Y * ν_vec
...
indic11 = Expression('(near(x[0], a, tol) and x[1] <= 0 and x[1] >= -(Len0 + G*t) ) ? 1: 0', degree=4,  a=a, tol=tol, Len0=L_0, G=G, t=0)
indic12 = Expression('x[1] >= -(Len0 + G*t)  ? 1: 0', degree=4, Len0=L_0, G=G, t=0)

#Approx indicator function x Fy
indic2 = Expression('Fy*(1/ppi)*(atan((x[1] +(Len0 + G*t - dlt))/(5*epss)) - atan((x[1] +(Len0 + G*t-dlt -dLenY))/(5*epss) ))', degree=6, Len0=L_0, ppi = π, epss = ε_s, G=G, Fy=Fy, dLenY=δLy, dlt=δlt, t=0)

indic21 = Expression('(near(x[0], a, tol) and x[1] <= -(Len0 + G*t-dlt-dLenY) and x[1] >= -(Len0 + G*t-dlt) ) ? Fy: 0', degree=4, a=a, tol=tol, Len0=L_0, G=G, dLenY=δLy, dlt = δlt, Fy=Fy, t=0)
...
# Use the same indicator functions as for the BC on the other side of the domain since it only concerns the z coordinate?
# Define variational problem for step 1

a1 = (θ + by) * Y * v_2*dx + dt * dot(Dy * grad(Y),grad(v_2)) * dx + dt * (ρ/(KM+YL_n))*Y * v_2 * dx
#- ky*(by**2)*Y/((1 + ky*by*XLs)**2) * (XLs - XL_n) * v_2*dx
b1 = (θ + by) * YL_n *v_2*dx + dt * indic2 *v_2*ds(1) + dt * indic2 *v_2*ds(3)

# Variational problem for step 2

a2 = (θ + bx/(1 + kbx*YL_n)) * X*v_1*dx - (kbx*bx /((1 + kbx*YL_n)**2))* (YLs - YL_n) * X* v_1*dx + dt * dot(Dx * grad(X),grad(v_1)) * dx + dt * indic1*α *X *v_1*ds(1) + dt * indic1*α *X *v_1*ds(3)
...
xdmffile_X = XDMFFile('Zn_lin_dec/Zn.xdmf')
xdmffile_Y = XDMFFile('DMA_lin_dec/DMA.xdmf')
